...
export default function ReportsPage() {
  ...
  const filteredDetail    = detailData.filter(l => l.noPolisi.toLowerCase().includes(searchTerm.toLowerCase()) || l.pmks.toLowerCase().includes(searchTerm.toLowerCase()))
  const filteredCancels   = cancelData.filter(l => l.noPolisi.toLowerCase().includes(searchTerm.toLowerCase()) || l.pmks.toLowerCase().includes(searchTerm.toLowerCase()))
  const filteredAnomalies = anomalyData.filter(l => l.noPolisi.toLowerCase().includes(searchTerm.toLowerCase()) || l.pmks.toLowerCase().includes(searchTerm.toLowerCase()))
          ...
          {loading ? (
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y">
                {summaryData.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">Tidak ada data untuk periode ini.</td></tr>}
                {summaryData.map(row => (
            ...
            <table className="w-full text-xs text-left">
              <thead className="bg-muted/50 text-muted-foreground uppercase font-semibold">
                <tr>
                  <th className="px-4 py-3">No Polisi</th><th className="px-4 py-3">PMKS</th><th className="px-4 py-3">Material</th>
                  <th className="px-4 py-3">Masuk</th><th className="px-4 py-3">Selesai</th>
                  <th className="px-4 py-3 text-right">Pending</th><th className="px-4 py-3 text-right">Bongkar</th>
                  <th className="px-4 py-3 text-right font-bold">Total</th><th className="px-4 py-3 text-right">Metrics</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {filteredDetail.length === 0 && <tr><td colSpan={9} className="px-6 py-12 text-center text-muted-foreground">Tidak ada data.</td></tr>}
                {filteredDetail.map(log => (
                  <tr key={log.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3"><span className={`inline-block px-1.5 py-0.5 rounded border text-[10px] font-bold ${MATERIAL_BADGE[log.materialType]}`}>{log.materialType}</span></td>
                    ...
                    <td className="px-4 py-3 text-right text-muted-foreground">
                      {log.materialType === "CPO" ? `FFA ${fmtNum(log.ffa)} / M ${log.moisture ?? "-"}%` : `KA ${fmtNum(log.ka)} / KK ${fmtNum(log.kk)} / St ${fmtNum(log.stone)}`}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-muted/50 text-muted-foreground uppercase text-xs font-semibold">
                <tr>
                  <th className="px-5 py-3">PMKS</th><th className="px-5 py-3">Material</th>
                  <th className="px-5 py-3 text-center">Kendaraan</th>
                  <th className="px-5 py-3 text-right">Avg FFA</th><th className="px-5 py-3 text-right">Avg Moisture</th>
                  <th className="px-5 py-3 text-right">Avg KA</th><th className="px-5 py-3 text-right">Avg KK</th><th className="px-5 py-3 text-right">Avg Stone</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {supplierData.length === 0 && <tr><td colSpan={8} className="px-6 py-12 text-center text-muted-foreground">Tidak ada data untuk periode ini.</td></tr>}
                {supplierData.map((row, i) => (
                  <tr key={i} className="hover:bg-muted/30 transition-colors">
                    <td className="px-5 py-3 font-semibold uppercase">{row.pmks}</td>
                    <td className="px-5 py-3"><span className={`inline-block px-1.5 py-0.5 rounded border text-xs font-bold ${MATERIAL_BADGE[row.materialType]}`}>{row.materialType}</span></td>
            ...
            <>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-muted/50 text-muted-foreground uppercase text-xs font-semibold">
                  <tr>
                    <th className="px-5 py-3">No Polisi</th><th className="px-5 py-3">PMKS</th><th className="px-5 py-3">Material</th>
                    <th className="px-5 py-3">Waktu Masuk</th><th className="px-5 py-3">Waktu Batal</th>
                    <th className="px-5 py-3">Petugas</th><th className="px-5 py-3">Alasan / Keterangan</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {filteredCancels.length === 0 && <tr><td colSpan={7} className="px-6 py-12 text-center text-muted-foreground">Tidak ada pembatalan untuk periode ini.</td></tr>}
                  {filteredCancels.map(log => (
                    <tr key={log.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-5 py-3"><span className={`inline-block px-1.5 py-0.5 rounded border text-xs font-bold ${MATERIAL_BADGE[log.materialType]}`}>{log.materialType}</span></td>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-muted/50 text-muted-foreground uppercase text-xs font-semibold">
                <tr>
                  <th className="px-6 py-4">#</th><th className="px-6 py-4">Nama Petugas</th>
              ...
              <tbody className="divide-y">
                {operatorData.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center text-muted-foreground">Tidak ada data aktivitas untuk periode ini.</td></tr>}
            ...
            <>
              ...
              <table className="w-full text-xs text-left">
                <thead className="bg-muted/50 text-muted-foreground uppercase font-semibold">
                  <tr>
                    <th className="px-4 py-3">No Polisi</th><th className="px-4 py-3">PMKS</th><th className="px-4 py-3">Material</th>
                    <th className="px-4 py-3">Masuk</th><th className="px-4 py-3">Parameter Melebihi Batas</th>
                    <th className="px-4 py-3 text-right">FFA</th><th className="px-4 py-3 text-right">M/KA</th>
                    <th className="px-4 py-3 text-right">KK</th><th className="px-4 py-3 text-right">Stone</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {filteredAnomalies.length === 0 && <tr><td colSpan={9} className="px-6 py-12 text-center text-muted-foreground">Tidak ada anomali. Semua kendaraan memenuhi standar kualitas ✓</td></tr>}
                  {filteredAnomalies.map(log => {
                    const flagParams = new Set(log.flags.map((f: any) => f.param))
                    const fc = (p: string) => flagParams.has(p) ? "text-destructive font-bold" : "text-muted-foreground"
                    return (
                      <tr key={log.id} className="hover:bg-destructive/5 transition-colors">
                        ...
                        <td className={`px-4 py-3 text-right tabular-nums ${fc("FFA")}`}>{fmtNum(log.ffa)}</td>
                        <td className={`px-4 py-3 text-right tabular-nums ${flagParams.has("KA") || flagParams.has("Moisture") ? "text-destructive font-bold" : "text-muted-foreground"}`}>
                          {log.materialType === "CPO" ? (log.moisture ?? "-") + "%" : fmtNum(log.ka)}
                        </td>
                        <td className={`px-4 py-3 text-right tabular-nums ${fc("KK")}`}>{fmtNum(log.kk)}</td>
                        <td className={`px-4 py-3 text-right tabular-nums ${fc("Stone")}`}>{fmtNum(log.stone)}</td>
                      </tr>
                    )
                  })}
                </tbody>
