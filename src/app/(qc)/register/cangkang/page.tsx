...
export default function RegisterCangkangPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append("materialType", "CANGKANG")
      ...
      <div className="border rounded-lg shadow-sm bg-card text-card-foreground p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-muted/50 -mx-6 px-6 py-4 border-y">
            <h3 className="font-semibold text-sm mb-4">Quality Metrics (Cangkang)</h3>
          ...
          >
            {isSubmitting ? "Registering..." : "Register Vehicle (Cangkang)"}
          </button>
        </form>
      </div>
    </div>
