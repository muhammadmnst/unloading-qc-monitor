...
export default function RegisterCPOPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append("materialType", "CPO")
      ...
      <div className="border rounded-lg shadow-sm bg-card text-card-foreground p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-muted/50 -mx-6 px-6 py-4 border-y">
            <h3 className="font-semibold text-sm mb-4">Quality Metrics (CPO)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
              <div className="space-y-2">
                <label htmlFor="ffa" className="text-sm font-medium leading-none text-muted-foreground">FFA (%)</label>
                <input 
                  className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" 
                  id="ffa" name="ffa" type="number" step="0.01" placeholder="0.00" 
                />
              </div>
              <div className="space-y-2">
                <label htmlFor="moisture" className="text-sm font-medium leading-none text-muted-foreground">Moisture (%)</label>
                <input 
                  className="flex h-10 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" 
                  id="moisture" name="moisture" type="text" placeholder="e.g. < 0.5" 
                />
              </div>
            </div>
          </div>
          ...
          >
            {isSubmitting ? "Registering..." : "Register Vehicle (CPO)"}
          </button>
        </form>
      </div>
    </div>
